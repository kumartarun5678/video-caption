...
export const uploadVideo = async (file: Express.Multer.File): Promise<VideoFile> => {
...
export const processVideo = async (
  videoId: string, 
  operation: 'audio' | 'thumbnail' | 'preview'
): Promise<string> => {
...
const generateThumbnail = (videoPath: string, thumbnailPath: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    ffmpeg(videoPath)
      .screenshots({
        timestamps: ['00:00:01'],
...
const generatePreview = (videoPath: string, previewPath: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    ffmpeg(videoPath)
